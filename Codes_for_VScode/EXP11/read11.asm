DATA SEGMENT
	;提示消息
	MSG1    DB 	?,?,'-MAKA BAKA';字符串1
	SLENGTH1 EQU 	$-MSG1
	MSG2    DB 	?,?,'-WUXI DIXI';字符串2
	SLENGTH2 EQU	$-MSG2
	;计数器
	COUNT   DB 0        
	CLOCK   DB 0	
    ...
    T       DB 10       ;输入时间的数值
	N 		DB 18       ;转换为计数次数
    TXT1    DB "Please input TIME in units of 0.1s(5<=TIME<=50):$"  ;文本提示1
...
;检验
    ...
    CALL  ASCtoNUM      ;把缓存区ASCII码转换成NUM
	CALL  VALUEerror    ;判断时间是否在范围内	
;将时间转换为计数次数储存
        PUSH SI
        PUSH AX
        PUSH BX
...
AGAIN:
	CALL FAR PTR DISP1
	CMP KEYN,LIM1       ;与LIM1比较
	JAE NEXT
	CMP CLOCK,LIM2      ;与LIM2比较
	JAE NEXT
	JMP AGAIN
NEXT:
;恢复09H中断	
...
;设置显示模式40*25黑白
	...
	MOV DX,0 ;行号为0，列号为0
REPT:
;附加任务，LIM2次显示SUN后立即停止返回DOS
	CMP CLOCK,LIM2;与LIM2比较
	JB 	K1
	JMP D_EXIT1
;附加任务，LIM1次键盘后立即停止返回DOS
K1:	
	CMP KEYN,LIM1;与LIM1比较
	JB 	K2
	JMP D_EXIT1
...
NEXTC1:
    ...
    LODSB                 
    MOV AH, 0EH             ;置AH寄存器为0EH，表示显示字符和属性。 
    MOV BX, 1
    INT 10H
    LOOP NEXTC1
;弹出 
